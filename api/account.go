...
type GormResponseexport struct {
	Code     int         `json:"code"`
	Message  string      `json:"msg"`
...
var gormResponseexport GormResponseexport
...
func Importaddress(c *gin.Context) {
	...
	json.Unmarshal([]byte(string(mnemonic)), &msg)
	fmt.Println(msg.Msg)
	master, err := hdwallet.NewKey(
		hdwallet.Mnemonic(msg.Msg),
	)
	if err != nil {
...
func Exportaddress(c *gin.Context) {
	...
	fmt.Println("BTC私钥：", btcwif)
	fmt.Println("BTC: ", address, addressP2WPKH, addressP2WPKHInP2SH)
	gormResponseexport.Code = http.StatusOK
	gormResponseexport.Message = msg.Msg
	gormResponseexport.Message1 = btcwif
	gormResponseexport.Data = addressP2WPKH
	c.JSON(http.StatusOK, gormResponseexport)
}
